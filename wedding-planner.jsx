...
// free-typing input for a member list — parses to an array only on blur so commas don't fight the cursor
function MembersInput({ value, onChange, width }) {
  ...
  return (
    <input
      style={{ ...inputStyle, width: width || 260 }}
      value={text}
      onChange={(e) => setText(e.target.value)}
      onBlur={() => onChange(parseMembers(text))}
      placeholder="e.g. Nathan Mama, Vicky Mami, Prabhu"
    />
  );
}
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-10" style={{ background: C.ivory, color: C.ink, colorScheme: theme }}>
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-6">
          ...
          <div style={{ ...serif, fontSize: 30, fontWeight: 600 }}>Guest check-in</div>
        </div>
        ...
        {!data ? (
          ...
          <Card>
            <div style={{ ...serif, fontSize: 22, fontWeight: 600 }}>You're checked in! 🎉</div>
          ...
          <Card>
            ...
            {(sel.members || []).length > 0 && (
              <p className="text-xs mt-1" style={{ color: C.muted }}>
                Party: {(sel.rsvp === "yes" && Array.isArray(sel.confirmedMembers) && sel.confirmedMembers.length ? sel.confirmedMembers : sel.members).join(", ")}
              </p>
            )}
            ...
            <div className="mt-4">
              <div className="text-sm font-semibold mb-2">Would you like to provide the couple with a monetary gift? 💝</div>
              <div className="flex gap-2">
            ...
            {giftIntent === "yes" && (
              <div className="mt-4">
                <div className="text-sm font-semibold mb-2">How would you like to give?</div>
                <div className="flex gap-2">
            ...
            <div className="mt-5">
              <Btn onClick={submit} disabled={busy || !num(pax) || giftIntent === null || (giftIntent === "yes" && (!method || !num(amount)))}>
...
function GuestRSVP({ onBack, theme, locked }) {
  ...
  const pick = (g) => {
    ...
    setSelMembers(
      Array.isArray(g.confirmedMembers)
        ? g.confirmedMembers.filter((m) => (g.members || []).includes(m))
        : [...(g.members || [])]
    );
    setErr("");
  };
        ...
        g.id === sel.id
          ? coming === "yes"
            ? {
                ...
                rsvpAt: Date.now(),
                ...((sel.members || []).length > 0 ? { confirmedMembers: selMembers } : {}),
              }
                ...
                {(sel.members || []).length > 0 && (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {sel.members.map((m) => {
                        const on = selMembers.includes(m);
                        return (
                          <button
                            key={m}
                            onClick={() => {
                              const next = on ? selMembers.filter((x) => x !== m) : [...selMembers, m];
                              setSelMembers(next);
...
function Guests({ data, up, side }) {
  const [form, setForm] = useState({ name: "", side: side || "bride", group: "", invitedPax: 2, invitedBabies: 0, phone: "", members: "" });
  const [filter, setFilter] = useState("all");
  const [search, setSearch] = useState("");
  const [view, setView] = useState("list");
  const [editId, setEditId] = useState(null);

  // every existing group path and each of its parent levels, for the autocomplete dropdown
  ...
  const add = () => {
    ...
    const members = parseMembers(form.members);
    const pax = Math.max(num(form.invitedPax) || 1, members.length);
    const g = {
      id: uid(),
      name: form.name.trim(),
      side: side || form.side,
      ...
      invitedPax: pax,
      invitedBabies: Math.min(num(form.invitedBabies), pax),
    ...
    setForm({ name: "", side: side || form.side, group: form.group, invitedPax: 2, invitedBabies: 0, phone: "", members: "" });
  };

  // planners tick off who from a family is coming; confirmed pax follows the count
  const toggleMember = (g, m) => {
    const base = Array.isArray(g.confirmedMembers) ? g.confirmedMembers : [...(g.members || [])];
    const next = base.includes(m) ? base.filter((x) => x !== m) : [...base, m];
    patch(g.id, { confirmedMembers: next, confirmedPax: next.length });
  };
      ...
      <Card>
        ...
        <div className="grid md:grid-cols-6 gap-3 items-end">
          ...
          <Field label="Group / relation">
            <input style={inputStyle} list="group-suggestions" value={form.group} onChange={(e) => setForm({ ...form, group: e.target.value })} placeholder="Dad's family / Uncle Ravi's family — use / to nest" />
          </Field>
          ...
          <Field label="…of which babies 👶">
            <input style={inputStyle} type="number" min="0" value={form.invitedBabies} onChange={(e) => setForm({ ...form, invitedBabies: e.target.value })} />
          </Field>
        </div>
        <p className="text-xs mt-2" style={{ color: C.muted }}>
          Babies are counted in the invite (e.g. 4 pax, 1 baby) but excluded from the catering headcount — they don't take a seat's worth of food.
        </p>
        <div className="grid md:grid-cols-5 gap-3 items-end mt-3">
          <Field label="Family members (optional, comma-separated)" className="md:col-span-2">
            <input
              style={inputStyle}
              value={form.members}
              onChange={(e) => setForm({ ...form, members: e.target.value })}
              placeholder="e.g. Nathan Mama, Vicky Mami, Prabhu, Arvind, Pravin"
            />
          </Field>
        ...
        <p className="text-xs mt-2" style={{ color: C.muted }}>
          Name the people under this invite and the pax count follows automatically. Once they RSVP, you (or they, via
          the RSVP link) can tick exactly who's coming.
        </p>
      </Card>
        ...
        <Card key={g.id} style={{ padding: 14 }}>
          ...
          {(g.members || []).length > 0 && (
            <div className="flex flex-wrap items-center gap-1 mt-2">
              ...
              {g.members.map((m) => {
                ...
                const coming = known ? g.confirmedMembers.includes(m) : null;
                return (
                  <button
                    key={m}
                    onClick={() => g.rsvp === "yes" && toggleMember(g, m)}
          ...
          {editId === g.id && (
            <div className="flex flex-wrap gap-3 mt-3 items-end p-3" style={{ background: C.soft, border: `1px dashed ${C.gold}`, borderRadius: 8 }}>
              ...
              <Field label="Group / relation">
                <input style={{ ...inputStyle, width: 230 }} list="group-suggestions" value={g.group || ""} onChange={(e) => patch(g.id, { group: e.target.value })} placeholder="e.g. Dad's family / Uncle Ravi's family" />
              </Field>
              <Field label="Members (comma-separated)">
                <MembersInput
                  ...
                  onChange={(m) =>
                    patch(g.id, {
                      members: m.length ? m : undefined,
                      invitedPax: Math.max(num(g.invitedPax) || 1, m.length),
                    })
                  }
                />
              </Field>
              ...
              <Field label="…of which babies 👶">
                <input style={{ ...inputStyle, width: 90 }} type="number" min="0" value={g.invitedBabies} onChange={(e) => patch(g.id, { invitedBabies: e.target.value })} />
              </Field>
              <Field label="Phone">
  ...
  const importGuests = async (file) => {
    try {
      ...
      rows.forEach((r) => {
        ...
        const members = kMembers ? parseMembers(String(r[kMembers])) : [];
              ...
              return (
                <div key={g.id} className="flex flex-wrap items-center gap-2 p-2" style={{ background: C.soft, border: `1px solid ${C.line}`, borderRadius: 8 }}>
                  ...
                  <div className="ml-auto">
                    <Btn kind="danger" small onClick={() => patch(g.id, { checkedInAt: null, checkedInPax: null, checkedInBabies: null })}>
                      Undo check-in
                    </Btn>
                  </div>
                </div>
