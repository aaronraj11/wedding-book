...
const TRASH_DAYS = 30;
const trashFresh = (t) => t && t.deletedAt && Date.now() - t.deletedAt < TRASH_DAYS * 86400000;
const pushTrash = (data, kind, item) => [
...
const CHART_COLORS = ["#6B8E7B", "#BC9459", "#8FA8C9", "#B48EAD", "#D9A66C", "#7FA8A0", "#B57373", "#9A8E82"];

function Donut({ slices, size = 150, stroke = 26, centerLabel, centerSub }) {
  ...
  return (
    <svg width={size} height={size} style={{ flexShrink: 0 }}>
      <circle cx={cx} cy={cx} r={r} fill="none" stroke={C.neutral} strokeWidth={stroke} />
      ...
      <text x={cx} y={cx - 1} textAnchor="middle" style={{ ...serif, fontSize: 22, fontWeight: 700, fill: C.ink }}>
        {centerLabel !== undefined ? centerLabel : total}
      </text>
      <text x={cx} y={cx + 16} textAnchor="middle" style={{ fontSize: 10, fill: C.muted }}>
        {centerSub || "pax"}
      </text>
    </svg>
  );
...
function ChartBlock({ title, slices }) {
  const total = slices.reduce((s, x) => s + x.value, 0);
  return (
    <div className="p-3" style={{ background: C.soft, border: `1px solid ${C.line}`, borderRadius: 12 }}>
      <div className="text-xs uppercase tracking-wider mb-2 text-center" style={{ color: C.muted }}>
        {title}
      </div>
      {total === 0 ? (
        ...
        <>
          <div className="flex justify-center">
            <Donut slices={slices.filter((s) => s.value > 0)} />
          </div>
            ...
            {slices
              ...
              .map((s) => (
                <div key={s.label} className="flex items-center gap-2 text-xs">
                  ...
                  <span className="ml-auto" style={{ color: C.muted, whiteSpace: "nowrap" }}>
    ...
    <div className="grid gap-4">
      ...
      {stats.guestCount > 0 &&
        (() => {
          ...
          const bridePax = data.guests.filter((g) => g.side === "bride").reduce((s, g) => s + paxOf(g), 0);
          const groomPax = data.guests.filter((g) => g.side === "groom").reduce((s, g) => s + paxOf(g), 0);
          const groupSlices = (sideKey) => {
            const map = {};
            data.guests
          ...
          return (
            <Card>
              <div style={{ ...serif, fontSize: 20, fontWeight: 600 }}>📊 Guest distribution</div>
              <p className="text-xs mt-1 mb-3" style={{ color: C.muted }}>
                Counted in invited pax (babies included). The side charts break each guest list down by group /
                relation — tune groups with the 🏷️ Groups panel in Guests & RSVP.
              </p>
              <div className={side ? "grid gap-4" : "grid md:grid-cols-3 gap-4"}>
                {!side && (
                  <ChartBlock
                ...
                {(!side || side === "bride") && <ChartBlock title="Bride's side — by group" slices={groupSlices("bride")} />}
                {(!side || side === "groom") && <ChartBlock title="Groom's side — by group" slices={groupSlices("groom")} />}
              </div>
            </Card>
          );
        })()}
...
function Guests({ data, up, side }) {
  ...
  const remove = (id) => {
    ...
    up({ guests: data.guests.filter((x) => x.id !== id), trash: pushTrash(data, "guest", g) });
  };
...
function DataPanel({ data, up }) {
  ...
  const restoreTrash = (t) => {
    ...
    up({ [key]: [t.item, ...(data[key] || [])], trash: (data.trash || []).filter((x) => x.id !== t.id) });
    setMsg(`Restored ${trashName(t)}.`);
  };
  ...
  return (
    <div className="grid gap-4">
      <Card>
        ...
        <div className="text-sm mt-3 grid gap-2" style={{ wordBreak: "break-all" }}>
          {[
            ["👥 Team link (opens your wedding's sign-in):", `${location.origin}${location.pathname}?w=${WEDDING}`],
            ["💌 RSVP link for guests (locked — they see only the RSVP page):", `${location.origin}${location.pathname}?w=${WEDDING}&p=rsvp`],
            ["🎟️ Check-in link for the door (locked — kiosk mode for a tablet/laptop):", `${location.origin}${location.pathname}?w=${WEDDING}&p=checkin`],
          ].map(([label, url]) => (
      ...
      <Card>
        <div style={{ ...serif, fontSize: 18, fontWeight: 600 }}>🗑️ Recently deleted</div>
        <p className="text-xs mt-1 mb-3" style={{ color: C.muted }}>
          Deleted guests, vendors, tasks, caterers and gifts rest here for {TRASH_DAYS} days before disappearing for
          good — restore anything with one click.
        </p>
        {(data.trash || []).filter(trashFresh).length === 0 ? (
          <span className="text-sm" style={{ color: C.muted }}>
            Nothing in the trash. 🎉
          </span>
        ) : (
          <div className="grid gap-2">
            {(data.trash || []).filter(trashFresh).map((t) => (
              <div key={t.id} className="flex flex-wrap items-center gap-2 p-2" style={{ background: C.soft, border: `1px solid ${C.line}`, borderRadius: 8 }}>
                <Pill>{TRASH_LABELS[t.kind] || t.kind}</Pill>
                <span className="text-sm font-medium">{trashName(t)}</span>
                <span className="text-xs" style={{ color: C.muted }}>
                  deleted {new Date(t.deletedAt).toLocaleDateString("en-MY", { day: "numeric", month: "short" })} ·
                  purges in {Math.max(1, Math.ceil(TRASH_DAYS - (Date.now() - t.deletedAt) / 86400000))}d
                </span>
                <div className="ml-auto flex gap-2">
                  <Btn small onClick={() => restoreTrash(t)}>
                    ↩ Restore
                  </Btn>
                  <Btn kind="danger" small onClick={() => purgeTrash(t)}>
                    Delete forever
                  </Btn>
                </div>
              </div>
  ...
  return (
    <Card>
      ...
      <div className="text-center mb-4">
        ...
        {(() => {
          const t = branchStats(pool);
          return (
            <div className="text-sm mt-3" style={{ color: C.muted }}>
              Combined total: <b style={{ color: C.green }}>{t.pax} pax</b> · {t.invites} invites
              {t.babies > 0 ? ` · ${t.babies} 👶` : ""} · {t.attending} attending
            </div>
          );
        })()}
      </div>
...
function Catering({ data, up, stats }) {
  ...
  const remove = (id) => {
    const c = data.caterers.find((x) => x.id === id);
    up({ caterers: data.caterers.filter((x) => x.id !== id), trash: pushTrash(data, "caterer", c) });
  };
...
function Budget({ data, up, stats }) {
  ...
  const remove = (id) => {
    ...
    up({ budget: data.budget.filter((x) => x.id !== id), trash: pushTrash(data, "budget", b) });
  };
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Card style={{ padding: 16 }}>
          ...
          <div className="flex items-center gap-1">
            <span className="text-sm" style={{ color: C.muted }}>RM</span>
            <input
              style={{ ...inputStyle, fontSize: 18, fontWeight: 600, padding: "4px 8px" }}
              type="number"
              min="0"
              value={data.budgetTarget || ""}
              onChange={(e) => up({ budgetTarget: e.target.value })}
              placeholder="set your total"
            />
          </div>
          <div className="text-xs mt-1" style={{ color: C.muted }}>
            {RM(stats.budgeted)} allocated across vendors
          </div>
        </Card>
        <Stat
          ...
          tone={stats.actual > (stats.budgetTarget || stats.budgeted) && (stats.budgetTarget || stats.budgeted) > 0 ? C.red : C.ink}
          sub={
            (stats.budgetTarget || stats.budgeted) > 0
...
function Todos({ data, up }) {
  ...
  const remove = (id) => {
    ...
    up({ todos: todos.filter((x) => x.id !== id), trash: pushTrash(data, "todo", t) });
  };
...
function Gifts({ data, up, side }) {
  ...
  const removeExtra = (id) => {
    const x = data.extraGifts.find((e) => e.id === id);
    up({ extraGifts: data.extraGifts.filter((e) => e.id !== id), trash: pushTrash(data, "extraGift", x) });
  };
